...
(library (scheme2k io port (0 9 3))
  (export byte-lines->port lines->port open-input-nowhere open-output-nowhere
          ...
          base64-string->bytevector bytevector->base64-string put-base64
...
;; convert final 1 byte to base64 and append it to wbuf
(define (base64-append/u8 wbuf u8)
  (let ((u12 (fx<< u8 4)))
    (bytespan-insert-right/u8! wbuf (u6->base64-byte (fx>> u12 6)))
    (bytespan-insert-right/u8! wbuf (u6->base64-byte u12))
    (bytespan-insert-right/u8! wbuf 61)   ; #\=
    (bytespan-insert-right/u8! wbuf 61))) ; #\=


;; convert final 2 bytes to base64 and append them to wbuf
(define (base64-append/u16 wbuf u16)
  (let ((u18 (fx<< u16 2)))
    (bytespan-insert-right/u8! wbuf (u6->base64-byte (fx>> u18 12)))
    (bytespan-insert-right/u8! wbuf (u6->base64-byte (fx>> u18 6)))
    (bytespan-insert-right/u8! wbuf (u6->base64-byte u18))
    (bytespan-insert-right/u8! wbuf 61))) ; #\=


;; convert 3 bytes to base64 and append them to wbuf
(define (base64-append/u24 wbuf u24)
  (bytespan-insert-right/u8! wbuf (u6->base64-byte (fx>> u24 18)))
  (bytespan-insert-right/u8! wbuf (u6->base64-byte (fx>> u24 12)))
  (bytespan-insert-right/u8! wbuf (u6->base64-byte (fx>> u24 6)))
  (bytespan-insert-right/u8! wbuf (u6->base64-byte u24)))
