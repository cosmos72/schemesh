  ...
  (protocol
    (lambda (args->new)
      (lambda (inner pred close-inner?)
         ...
         ;; until either (pred element) returns truish, or wrapped reader is exhausted
         ((args->new %filter-reader-get #f (and close-inner? %filter-inner-close))
            inner pred))))
  (nongenerative %filter-reader-7c46d04b-34f4-4046-b5c7-b63753c1be40))
...
(define make-filter-reader
  (case-lambda
    ((inner pred close-inner?)
      ...
      (assert* 'make-filter-reader (logbit? 1 (procedure-arity-mask pred)))
      (%make-filter-reader inner pred close-inner?))
    ((inner pred)
  ...
  (let-values (((obj ok?) (obj-reader-get (filter-reader-inner rx))))
    (cond
      ((not ok?) ;; inner reader is exhausted
        (values obj ok?))
      ((not ((filter-reader-pred rx) obj)) ;; pred tells to discard obj => iterate
