...
(library-reexport (schemesh minimal (1 0 1))
...
;;
(library-reexport (schemesh rnrs (1 0 1))
  (import
...
(library-reexport (schemesh (1 0 1))
  (import
    (scheme2k bootstrap)
    (scheme2k containers)
    (scheme2k containers replacements) ;; intentionally conflicts with some R6RS and Chez Scheme functions, because it is intended to replace them.
...
(eval-when (eval)
  (let ()
    ...
    (when (top-level-bound? 'reader?)
      (eval '(import (scheme2k io obj))))
    ...
    (when (top-level-bound? 'json-reader?)
      (eval '(import (scheme2k io))))
    (when (top-level-bound? 'wire-reader?)
      (eval '(import (scheme2k ipc))))
    (when (top-level-bound? 'process-reader?)
      (eval '(import (scheme2k os))))
