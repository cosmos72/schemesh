...
(define-syntax with-while-until
  (syntax-rules (while until)
    ...
    ((_ body)
      body)
    ((_ while pred body ...)
      (and pred (with-while-until body ...)))
    ((_ until pred body ...)
      (and (not pred) (with-while-until body ...)))
    ((_ body1 body2 ...)
...
(define-syntax for
  (lambda (stx)
    (syntax-case stx ()
      ((_ () body ...)
        #`(let for-loop0 ()
            (and (with-while-until body ...) (for-loop0))))
      ((_ var iterator body ...)
        (identifier? #'var)
        #`(iterate iterator (lambda0 (_ var) (with-while-until body ...))))
      ((_ ((var iterator)) body ...)
        #`(iterate iterator (lambda0 (_ var) (with-while-until body ...))))
