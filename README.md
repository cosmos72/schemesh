...
There is no need to memorize the correspondence between shell syntax and its corresponding Scheme counterpart:
you can type `(values '{shell syntax})` or `(expand '{shell syntax})` or `(values {shell syntax})`
(note: **no** quotes in the last expression) and inspect the Scheme source or objects generated by shell syntax:
```lisp
...
> (values {make > log && make install >> log && echo done})
(sh-and (sh-cmd* "make" 1 '> "log") (sh-cmd* "make" "install" 1 '>> "log") (sh-cmd "echo" "done"))
```
