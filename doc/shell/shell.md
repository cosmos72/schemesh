...
To see how shell syntax is parsed to `(shell)` macro, evaluate `(begin '{some shell command})`
...
Shell syntax does **not** allow spaces between a file descriptor number and the following redirection operator.
...
Again, shell syntax does **not** allow spaces between a file descriptor number and the following redirection operator.
