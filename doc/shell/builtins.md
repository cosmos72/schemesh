...
Return success if the directory is successfully changed, otherwise raise an exception.<br/>
Merely returning failure was considered too lenient, because executing commands in the wrong directory
can easily have unintended effects, such as removing the wrong files or directories.

Note: each job has its own current directory: initially it is inherited from parent job, but can be changed independently.<br/>
...
Without arguments, display the current file creation mask as four octal digits,
for example `0022`. With one argument, set the mask to the specified octal value
between `000` and `777`. Only digits `0` through `7` are accepted; symbolic modes
and options such as `-S` are not supported.
...
```
┌───┬─────────┬───────────┬────────────┬────────────┬────────────┬────────────┬───────────┬────────────┬──────────┬───┬─────┐
│id │ device  │mount-point│bytes-total │ bytes-free │bytes-avail │inodes-total│inodes-free│inodes-avail│block-size│dev│flags│
├───┼─────────┼───────────┼────────────┼────────────┼────────────┼────────────┼───────────┼────────────┼──────────┼───┼─────┤
│ 26│udev     │/dev       │ 16668770304│ 16668770304│ 16668770304│     4069524│    4068966│     4068966│      4096│  6│ 4098│
│ 28│tmpfs    │/run       │  3359391744│  3358326784│  3358326784│     4100817│    4099843│     4099843│      4096│ 25│ 4106│
│ 29│/dev/sda2│/          │ 82228940800│ 54430990336│ 50283610112│     1261568│     939586│      939586│      4096│  2│ 4096│
└───┴─────────┴───────────┴────────────┴────────────┴────────────┴────────────┴───────────┴────────────┴──────────┴───┴─────┘
```
