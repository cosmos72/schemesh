...
static void run_test_scheme(unsigned long* run_n, unsigned long* failed_n, const char* test_file) {
...
static void run_tests_scheme(unsigned long* run_n, unsigned long* failed_n) {
  const char* test_files[] = {"test/data1.ss", "test/data2.ss"};
  ...
  for (unsigned i = 0; i < N_OF(test_files); i++) {
    run_test_scheme(run_n, failed_n, test_files[i]);
  }
}
