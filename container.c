...
static void define_span_functions(void) {
  ...
  eval("(let ()\n"
       ...
       "     (mutable len span-end span-end-set!)\n"
       "     (mutable vec span-vec span-vec-set!))\n"
       "  (nongenerative #{%span m8umd7clk52rq9do8d38t1sab-0}))\n"
       "\n"
       ...
       "  (vector-ref (span-vec sp) n)))\n"
       "\n"
       "(set! span-set! (lambda (sp n val)\n"
       "  (assert (fx<? n (span-end sp)))\n"
       "  (vector-set! (span-vec sp) n val)))\n"
       "\n"
       ...
       /* no optimized function to fill only up to span-end,
        * so fill up to span-capacity */
       "  (vector-fill! (span-vec sp) val)))\n"
       "\n"
       "(set! span-fill-range! (lambda (sp start n val)\n"
       "  (assert (fx<=? (fx+ src n) (span-end sp)))\n"
       "  (vector-fill-range! (span-vec sp) start n val)))\n"
       "\n"
       ...
       "  (assert (fx<=? (fx+ src-start n) (span-end src)))\n"
       "  (assert (fx<=? (fx+ dst-start n) (span-end dst)))\n"
       ...
       "  (span-end-set! sp n)))\n"
       "\n"
       "(set! span-append! (lambda (sp . elements)\n"
       "  (unless (null? elements)\n"
       "    (let ((elem-n (length elements))\n"
       "          (pos    (span-end sp)))\n"
       "      (span-length-set! sp (fx+ pos elem-n))\n"
...
static void define_bytespan_functions(void) {
  ...
  eval("(let ()\n"
       ...
       "\n"
       "(set! bytespan-u8-append! (lambda (sp . elements)\n"
       "  (unless (null? elements)\n"
       "    (let ((elem-n (length elements))\n"
       "          (pos    (bytespan-len sp)))\n"
       "      (bytespan-length-set! sp (fx+ pos elem-n))\n"
       "      (list-iterate elements\n"
       "        (lambda (elem)\n"
...
static void define_charspan_functions(void) {
  ...
  eval("(let ()\n"
       ...
       "\n"
       "(set! charspan-append! (lambda (sp . elements)\n"
       "  (unless (null? elements)\n"
       "    (let ((elem-n (length elements))\n"
       "          (pos    (charspan-len sp)))\n"
       "      (charspan-length-set! sp (fx+ pos elem-n))\n"
       "      (list-iterate elements\n"
       "        (lambda (elem)\n"
