...
static void define_span_functions(void) {
  ...
  eval("(let ()\n"
       ...
       "  (%span %make-span %span?)\n"
       "  (fields\n"
       "     (mutable len span-len span-len-set!)\n"
       "     (mutable vec span-vec span-vec-set!))\n"
       "  (nongenerative #{%span ddartshmtuppqpm7bv5l7h7jm-0}))\n"
       "\n"
       "(set! list->span (lambda (l)\n"
       "  (let ((vec (list->vector l)))\n"
       "    (%make-span (vector-length vec) vec))))\n"
       "\n"
       "(set! vector->span (lambda (vec)\n"
       "  (%make-span (vector-length vec) (vector-copy vec))))\n"
       "\n"
       "(set! make-span (lambda (n . obj)\n"
       "  (%make-span n (apply make-vector n obj))))\n"
       "\n"
       "(set! span->vector (lambda (obj)\n"
       "  (subvector (span-vec obj) (span-len obj))))\n"
       "\n"
       "(set! span (lambda objs\n"
       "  (list->span objs)))\n"
       "\n"
       "(set! span? (lambda (obj)\n"
       "  (%span? obj)))\n"
       "\n"
       "(set! span-length (lambda (arr)\n"
       "  (span-len arr)))\n"
       "\n"
       "(set! span-capacity (lambda (arr)\n"
       "  (vector-length (span-vec arr))))\n"
       "\n"
       "(set! span-empty? (lambda (arr)\n"
       "  (fxzero? (span-len arr))))\n"
       "\n"
       "(set! span-ref (lambda (arr n)\n"
       "  (assert (fx<? n (span-len arr)))\n"
       "  (vector-ref (span-vec arr) n)))\n"
       "\n"
       "(set! span-set! (lambda (arr n obj)\n"
       "  (assert (fx<? n (span-len arr)))\n"
       "  (vector-set! (span-vec arr) n obj)))\n"
       "\n"
       "(set! span-last (lambda (arr)\n"
       "  (assert (not (span-empty? arr)))\n"
       "  (vector-ref (span-vec arr) (fx1- (span-len arr)))))\n"
       "\n"
       "(set! span-fill! (lambda (arr obj)\n"
       /* no optimized function to fill only up to span-len,
        * so fill up to span-capacity */
       "  (vector-fill! (span-vec arr) obj)))\n"
       "\n"
       "(set! span-fill-range! (lambda (arr start n obj)\n"
       "  (assert (fx<=? (fx+ src n) (span-len arr)))\n"
       "  (vector-fill-range! (span-vec arr) start n obj)))\n"
       "\n"
       "(set! span-copy (lambda (src)\n"
       "  (let* ((n (span-length src))\n"
       "         (dst (make-span n)))\n"
       "    (vector-copy! (span-vec src) 0 (span-vec dst) 0 n)\n"
       "    dst)))\n"
       "\n"
       "(set! span-copy! (lambda (src src-start dst dst-start n)\n"
       "  (assert (fx<=? (fx+ src-start n) (span-len src)))\n"
       "  (assert (fx<=? (fx+ dst-start n) (span-len dst)))\n"
       "  (vector-copy! (span-vec src) src-start (span-vec dst) dst-start n)))\n"
       "\n"
       "(set! span-capacity-set! (lambda (arr n)\n"
       "  (assert (fx>=? n (span-len arr)))\n"
       "  (unless (fx=? n (span-capacity arr))\n"
       "    (let* ((len (span-len arr))\n"
       "           (old-vec (span-vec arr))\n"
       "           (new-vec (make-vector n)))\n"
       "      (vector-copy! old-vec 0 new-vec 0 len)\n"
       "      (span-vec-set! arr new-vec)))))\n"
       "\n"
       "(set! span-length-set! (lambda (arr n)\n"
       "  (when (fx>? n (span-capacity arr))\n"
       "    (let ((new-cap (fxmax 8 n (fx* 2 (span-capacity arr)))))\n"
       "      (span-capacity-set! arr new-cap)))\n"
       "  (span-len-set! arr n)))\n"
       "\n"
       "(set! span-append! (lambda (arr . elements)\n"
       "  (unless (null? elements)\n"
       "    (let ((elem-n (length elements))\n"
       "          (pos    (span-len arr)))\n"
       "      (span-length-set! arr (fx+ pos elem-n))\n"
       "      (list-iterate elements\n"
       "        (lambda (elem)\n"
       "          (span-set! arr pos elem)\n"
       "          (set! pos (fx1+ pos))))))))\n"
       "\n"
       "(set! span-iterate (lambda (arr proc)\n"
       "  (do ((i 0 (fx1+ i))\n"
       "       (n (span-len arr))\n"
       "       (v (span-vec arr)))\n"
  ...
  /**
   * (span-find) iterates on span elements from start to (fxmin (fx+ start n) (span-length arr)),
   * and returns the index of first span element that causes (predicate elem)
   * to return non-#f.
   * Returns #f if no such element is found.
   */
  eval("(define (span-find arr start n predicate)\n"
       "  (let ((ret #f))\n"
       "    (do ((i   start (fx1+ i))\n"
       "         (end (fxmin (fx+ start n) (span-length arr))))\n"
       "        ((or ret (fx>=? i end)) ret)\n"
       "      (when (predicate (span-ref arr i))\n"
       "        (set! ret i)))))\n");
}
...
static void define_bytespan_functions(void) {
  ...
  eval("(let ()\n"
       ...
       "\n"
       "(set! bytespan->bytevector (lambda (obj)\n"
       "  (let* ((n (bytespan-len obj))\n"
       "         (dst (make-bytevector n)))\n"
       "    (bytevector-copy! (bytespan-vec obj) 0 dst 0 n)\n"
       "    dst)))\n"
       "\n"
       "(set! bytespan (lambda objs\n"
       "  (list->bytespan objs)))\n"
       "\n"
       "(set! bytespan? (lambda (obj)\n"
       "   (%bytespan? obj)))\n"
       "\n"
       "(set! bytespan-length (lambda (arr)\n"
       "  (bytespan-len arr)))\n"
       "\n"
       "(set! bytespan-capacity (lambda (arr)\n"
       "  (bytevector-length (bytespan-vec arr))))\n"
       "\n"
       "(set! bytespan-underlying (lambda (arr)\n"
       "  (bytespan-vec arr)))\n"
       "\n"
       "(set! bytespan-empty? (lambda (arr)\n"
       "  (fxzero? (bytespan-len arr))))\n"
       "\n"
       "(set! bytespan-u8-ref (lambda (arr n)\n"
       "  (assert (fx<? n (bytespan-len arr)))\n"
       "  (bytevector-u8-ref (bytespan-vec arr) n)))\n"
       "\n"
       "(set! bytespan-u8-set! (lambda (arr n obj)\n"
       "  (assert (fx<? n (bytespan-len arr)))\n"
       "  (bytevector-u8-set! (bytespan-vec arr) n obj)))\n"
       "\n"
       "(set! bytespan-u8-last (lambda (arr)\n"
       "  (assert (not (bytespan-empty? arr)))\n"
       "  (bytevector-u8-ref (bytespan-vec arr) (fx1- (bytespan-len arr)))))\n"
       "\n"
       "(set! bytespan-fill! (lambda (arr obj)\n"
       /* no optimized function to fill only up to bytespan-len,
        * so fill up to bytespan-capacity */
       "  (bytevector-fill! (bytespan-vec arr) obj)))\n"
       "\n"
       "(set! bytespan-fill-range! (lambda (arr start n obj)\n"
       "  (assert (fx<=? (fx+ src n) (bytespan-len arr)))\n"
       "  (bytevector-fill-range! (bytespan-vec arr) start n obj)))\n"
       "\n"
       ...
       "\n"
       "(set! bytespan-capacity-set! (lambda (arr n)\n"
       "  (assert (fx>=? n (bytespan-len arr)))\n"
       "  (unless (fx=? n (bytespan-capacity arr))\n"
       "    (let* ((len (bytespan-len arr))\n"
       "           (old-vec (bytespan-vec arr))\n"
       "           (new-vec (make-bytevector n)))\n"
       "      (bytevector-copy! old-vec 0 new-vec 0 len)\n"
       "      (bytespan-vec-set! arr new-vec)))))\n"
       "\n"
       "(set! bytespan-length-set! (lambda (arr n)\n"
       "  (when (fx>? n (bytespan-capacity arr))\n"
       "    (let ((new-cap (fxmax 8 n (fx* 2 (bytespan-capacity arr)))))\n"
       "      (bytespan-capacity-set! arr new-cap)))\n"
       "  (bytespan-len-set! arr n)))\n"
       "\n"
       "(set! bytespan-u8-append! (lambda (arr . elements)\n"
       "  (unless (null? elements)\n"
       "    (let ((elem-n (length elements))\n"
       "          (pos    (bytespan-len arr)))\n"
       "      (bytespan-length-set! arr (fx+ pos elem-n))\n"
       "      (list-iterate elements\n"
       "        (lambda (elem)\n"
       "          (bytespan-u8-set! arr pos elem)\n"
       "          (set! pos (fx1+ pos))))))))\n"
       "\n"
       "(set! bytespan-iterate (lambda (arr proc)\n"
       "  (do ((i 0 (fx1+ i))\n"
       "       (n (bytespan-len arr))\n"
       "       (v (bytespan-vec arr)))\n"
   ...
   * (bytespan-length arr)), and returns the index of first bytespan element that causes
   * (predicate elem) to return non-#f. Returns #f if no such element is found.
   */
  eval("(define (bytespan-u8-find arr start n predicate)\n"
       "  (let ((ret #f))\n"
       "    (do ((i   start (fx1+ i))\n"
       "         (end (fxmin (fx+ start n) (bytespan-length arr))))\n"
       "        ((or ret (fx>=? i end)) ret)\n"
       "      (when (predicate (bytespan-u8-ref arr i))\n"
       "        (set! ret i)))))\n");
}
...
static void define_charspan_functions(void) {
  ...
  eval("(let ()\n"
       ...
       "\n"
       "(set! charspan->string (lambda (obj)\n"
       "  (substring (charspan-vec obj) 0 (charspan-len obj))))\n"
       "\n"
       "(set! charspan (lambda objs\n"
       "  (list->charspan objs)))\n"
       "\n"
       "(set! charspan? (lambda (obj)\n"
       "  (%charspan? obj)))\n"
       "\n"
       "(set! charspan-length (lambda (arr)\n"
       "  (charspan-len arr)))\n"
       "\n"
       "(set! charspan-capacity (lambda (arr)\n"
       "  (string-length (charspan-vec arr))))\n"
       "\n"
       "(set! charspan-empty? (lambda (arr)\n"
       "  (fxzero? (charspan-len arr))))\n"
       "\n"
       "(set! charspan-ref (lambda (arr n)\n"
       "  (assert (fx<? n (charspan-len arr)))\n"
       "  (string-ref (charspan-vec arr) n)))\n"
       "\n"
       "(set! charspan-set! (lambda (arr n obj)\n"
       "  (assert (fx<? n (charspan-len arr)))\n"
       "  (string-set! (charspan-vec arr) n obj)))\n"
       "\n"
       "(set! charspan-last (lambda (arr)\n"
       "  (assert (not (charspan-empty? arr)))\n"
       "  (string-ref (charspan-vec arr) (fx1- (charspan-len arr)))))\n"
       "\n"
       "(set! charspan-fill! (lambda (arr obj)\n"
       /* no optimized function to fill only up to charspan-len,
        * so fill up to charspan-capacity */
       "  (string-fill! (charspan-vec arr) obj)))\n"
       "\n"
       "(set! charspan-fill-range! (lambda (arr start n obj)\n"
       "  (assert (fx<=? (fx+ src n) (charspan-len arr)))\n"
       "  (substring-fill! (charspan-vec arr) start n obj)))\n"
       "\n"
       ...
       "\n"
       "(set! charspan-capacity-set! (lambda (arr n)\n"
       "  (assert (fx>=? n (charspan-len arr)))\n"
       "  (unless (fx=? n (charspan-capacity arr))\n"
       "    (let* ((len (charspan-len arr))\n"
       "           (old-vec (charspan-vec arr))\n"
       "           (new-vec (make-string n)))\n"
       "      (string-copy! old-vec 0 new-vec 0 len)\n"
       "      (charspan-vec-set! arr new-vec)))))\n"
       "\n"
       "(set! charspan-length-set! (lambda (arr n)\n"
       "  (when (fx>? n (charspan-capacity arr))\n"
       "    (let ((new-cap (fxmax 8 n (fx* 2 (charspan-capacity arr)))))\n"
       "      (charspan-capacity-set! arr new-cap)))\n"
       "  (charspan-len-set! arr n)))\n"
       "\n"
       "(set! charspan-append! (lambda (arr . elements)\n"
       "  (unless (null? elements)\n"
       "    (let ((elem-n (length elements))\n"
       "          (pos    (charspan-len arr)))\n"
       "      (charspan-length-set! arr (fx+ pos elem-n))\n"
       "      (list-iterate elements\n"
       "        (lambda (elem)\n"
       "          (charspan-set! arr pos elem)\n"
       "          (set! pos (fx1+ pos))))))))\n"
       "\n"
       "(set! charspan-iterate (lambda (arr proc)\n"
       "  (do ((i 0 (fx1+ i))\n"
       "       (n (charspan-len arr))\n"
       "       (v (charspan-vec arr)))\n"
       ...
       "    (write (substring (charspan-vec obj) 0 (charspan-len obj)) port)\n"
       "    (display #\\) port)))\n"
       ")\n");

  /**
   * (charspan-find) iterates on charspan elements from start to (fxmin (fx+ start n)
   * (charspan-length arr)), and returns the index of first charspan element that causes
   * (predicate elem) to return non-#f. Returns #f if no such element is found.
   */
  eval("(define (charspan-find arr start n predicate)\n"
       "  (let ((ret #f))\n"
       "    (do ((i   start (fx1+ i))\n"
       "         (end (fxmin (fx+ start n) (charspan-length arr))))\n"
       "        ((or ret (fx>=? i end)) ret)\n"
       "      (when (predicate (charspan-ref arr i))\n"
       "        (set! ret i)))))\n");
}
