...
(library (scheme2k containers span (1 0 2))
  (export
  ...
  (import
    ...
    (only (scheme2k bootstrap)         assert* assert-not* begin0 for fx<=?* generate-pretty-temporaries lambda0)
...
(define-syntax for-span
  (lambda (stx)
    (syntax-case stx ()
      ...
      ((_ elem sp body ...)
        (identifier? #'elem)
        #'(span-iterate sp (lambda0 (_ elem) body ...)))
      ((_ ((elem sp)) body ...)
        #'(span-iterate sp (lambda0 (_ elem) body ...)))
      ((_ ((elem sp) ...) body ...)
