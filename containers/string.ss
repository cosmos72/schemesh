...
(library (scheme2k containers string (1 0 2))
  (export
...
(define string-index
  (let ((c-string-index-ch    (foreign-procedure "c_string_index_ch"    (ptr ptr fixnum fixnum) ptr))
        (c-string-index-chars (foreign-procedure "c_string_index_chars" (ptr ptr fixnum fixnum) ptr)))
    (case-lambda
      ((str char/char-set/pred start end)
        ...
        (let ((key char/char-set/pred))
          (cond
            ...
            ((string? key)
              (c-string-index-chars str key start end))
            (else
              (assert* 'string-index (procedure? key))
              (assert* 'string-index (logbit? 1 (procedure-arity-mask key)))
              (do ((i start (fx1+ i)))
                ((or (fx>=? i end) (key (string-ref str i)))
                  (and (fx<? i end) i)))))))
...
(define string-index-right
  (let ((c-string-index-right-ch    (foreign-procedure "c_string_index_right_ch"    (ptr ptr fixnum fixnum) ptr))
        (c-string-index-right-chars (foreign-procedure "c_string_index_right_chars" (ptr ptr fixnum fixnum) ptr)))
    (case-lambda
      ((str char/char-set/pred start end)
        ...
        (let ((key char/char-set/pred))
          (cond
            ...
            ((string? key)
              (c-string-index-right-chars str key start end))
            (else
              (assert* 'string-index-right (procedure? key))
              (assert* 'string-index-right (logbit? 1 (procedure-arity-mask key)))
              (do ((i (fx1- end) (fx1- i)))
                  ((or (fx<? i start) (key (string-ref str i)))
                    (and (fx>=? i start) i)))))))
      ((str char/char-set/pred)
