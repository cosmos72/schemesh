...
#(
  ...
  (let ((rx (make-csv-reader
              (open-bytevector-input-port
                (string->utf8b
                  "a , b, c ,d,e\n\" x \", y ,\" \", 78.9 ,123e99\n\n")))))
    (all rx))                                           ((a " x " b "y" c " " d 789/10 e 1.23e101) ())
  ...
  ;; test umask: restore the original umask even if a test raises an exception.
  (let ((saved (c-umask)))
    (dynamic-wind
      void
      (lambda ()
  ...
  (let ((saved (c-umask)))
    (dynamic-wind
      ...
      (lambda ()
        (sh-run {umask 027})
        (list
          (for-list arg '("" "089" "1000" "-1" "+22" "#o22" "2/2" "2.0" " 22" "22 " "u=rwx" "-S")
...
B=2})                                                  ,@"#<void>"
  ...
  ;; test shell builtin 'get0'
  (sh-run/string
    {echo "{\"name\":\"main.c\"}
           {\"name\":\"test.c\"}" | get0 name})         "main.c\x0;test.c\x0;"
  ...
  (sh-run/string
    {$(==> datum-reader '(foo "bar")
         => to-json) | to text0})                       "bar\x0;"
